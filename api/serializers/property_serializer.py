...
class AmenitiesSerializer(serializers.ModelSerializer):
    balcony_value = serializers.CharField(
        source="get_balcony_display", read_only=True
    )
    water_supply_value = serializers.CharField(
        source="get_water_supply_display", read_only=True
    )
    
    gym_value = serializers.CharField(
        source="get_gym_display", read_only=True
    )
    non_veg_value = serializers.CharField(
        source="get_non_veg_display", read_only=True
    )
    security_value = serializers.CharField(
        source="get_security_display", read_only=True
    )
    viewer_value = serializers.CharField(
        source="get_viewer_display", read_only=True
    )
    class Meta:
        model = Amenities
        fields = (
...
class RentalSerializer(serializers.ModelSerializer):
    ...
    tenants_value = serializers.CharField(
        source="get_tenants_display", read_only=True
    )
    furnishing_value = serializers.CharField(
        source="get_furnishing_display", read_only=True
    )
    parking_value = serializers.CharField(
        source="get_furnishing_display", read_only=True
    )
...
class ScheduleSerializer(serializers.ModelSerializer):
    ...
    available_days_value = serializers.CharField(source="get_available_days_display",read_only=True)
...
class PropertyDetailSerializer(serializers.ModelSerializer):
    ...
    age = serializers.CharField(source="get_bhk_type_display")
    facing = serializers.CharField(source="get_bhk_type_display")
    bhk_type = serializers.CharField(source="get_bhk_type_display")
    membership_plan = serializers.CharField(source="get_membership_plan_display")
    condition_type = serializers.CharField(source="get_bhk_type_display")
